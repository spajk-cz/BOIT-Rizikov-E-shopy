...
/**
 * Wrapper kolem browser.runtime.sendMessage — vrací null místo throw,
 * aby UI code nemusel řešit try/catch u každého volání.
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('openCoiBtn').addEventListener('click', () => {
    browser.tabs.create({ url: 'https://coi.gov.cz/pro-spotrebitele/rizikove-e-shopy/' });
  });

  document.getElementById('openSoiBtn').addEventListener('click', () => {
    browser.tabs.create({ url: 'https://www.soi.sk/informacie-pre-verejnost/internetove-obchody/rizikove-internetove-obchody' });
  });
  ...
  document.getElementById('boitLink').addEventListener('click', () => {
    browser.tabs.create({ url: 'https://boit.cz' });
  });
});
